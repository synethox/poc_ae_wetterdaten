...
function clampMaxDate(d: string) {
...
export default function App() {
  ...
  const warmupStation = async (stationId: string, warmFrom: string, warmTo: string) => {
    ...
    try {
      await fetch(
        `/api/temperatures?station_id=${encodeURIComponent(stationId)}&from=${warmFrom}&to=${warmTo}`
      );
    } finally {
      setWarmingUp(false);
    }
    ...
    <div className="appShell">
      <Toolbar
        ...
        onChange={(patch) => {
          ...
          if (patch.from !== undefined) setFrom(clampMaxDate(patch.from));
          if (patch.to !== undefined) setTo(clampMaxDate(patch.to));
        }}
